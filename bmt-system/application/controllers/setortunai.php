...
class Setortunai extends Controller
{
    ...
    public function index()
    {
        ...
        echo $this->twig_lib->render('tpl/setortunai.php', $data);
    ...
    public function saldo()
    {
        $data           = $this->allfunct->securePost();
        $no_rekening    = $data['id'];
        $query = $this->db->query("SELECT ifnull(sum(accounttrans_value), 0) as jlh FROM tb_accounttrans WHERE accounttrans_user='".$no_rekening."' and accounttrans_type='01'");
        $data = $query->result_array();
        if ($query->num_rows() > 0) {
            $query1 = $this->db->query("SELECT ifnull(sum(accounttrans_value), 0) as jlh FROM tb_accounttrans WHERE accounttrans_user='".$no_rekening."' and accounttrans_type='02'");

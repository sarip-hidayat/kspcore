 ...
 */
/*----------------------------------------------------------*/
class Hitungbasil extends Controller
{
    ...
    public function index()
    {
        ...
        $data['tema'] = $this->tema;
        $data['page_title'] = "Perhitungan Bagi Hasil";
        ...
        $data['js_links'] = get_js(
            array(
            $assets_path .'/js/tool/hitungbasil.js',
        ...
        echo $this->twig_lib->render('tpl/tool/hitungbasil.php', $data);
        // $this->load->view('tool/hitungbasil',$data);
    }
    public function distribusiprofit()
    {
    ...
    public function transnilaivalue($type, $id, $tglawal, $tglakhir)
    {
        if ($id == "203") {
    ...
    public function rinciansaldorata()
    {
        $tglawal = $this->allfunct->revDate($this->input->post('tgl1'));
        $tglakhir = $this->allfunct->revDate($this->input->post('tgl2'));
        $hasil = $this->db->query("SELECT listakun_id,listakun_code,listakun_name,nama_produk FROM coa_listakun AS t1 INNER JOIN tb_akunperhimpunan AS t2 ON t2.akun=t1.listakun_id")->result();
    ...
    public function hitungbasilwadiah()
    {
        $pBasilW = 0;
        $tglawal = $this->allfunct->revDate($this->input->post('tgl1'));
        $tglakhir = $this->allfunct->revDate($this->input->post('tgl2'));
        ...
        $hasil = $this->db->query(
            "SELECT DISTINCT(basil_rek),basil_value,basil_code 
        							FROM tb_basil WHERE basil_value >'".$minsaldo."' AND 
        							basil_type=2 AND end_date='$tglakhir' AND basil_rek !='P001'"
        )->result();
        foreach ($hasil as $row) {
            $nomor_rekening = $row->basil_rek;
    ...
    public function hitungbasiltotol()
    {
        $pBasil = 0;
        $tglawal = $this->allfunct->revDate($this->input->post('tgl1'));
        $tglakhir = $this->allfunct->revDate($this->input->post('tgl2'));
    ...
    public function hitung()
    {
        ...
        /*$hasil = $this->db->query("SELECT accounttrans_date,accounttrans_user,COUNT(*)
									...
									GROUP BY accounttrans_user")->result();*/
        $hasil = $this->db->query(
            "SELECT accounttrans_date,accounttrans_user,COUNT(*) AS jlhtrans 
									FROM tb_accounttrans AS t1
									WHERE accounttrans_user !=0 AND accounttrans_date <='$tglakhir'	
									GROUP BY accounttrans_user"
        )->result();
        foreach ($hasil as $row) {
            ...
            for ($a=0; $a< $jhhari; $a++) {
                ...
                $query1 = $this->db->query(
                    "SELECT accounttrans_user,`accounttrans_date`,SUM(CASE WHEN accounttrans_type like '01' THEN accounttrans_value END) AS jlh1,

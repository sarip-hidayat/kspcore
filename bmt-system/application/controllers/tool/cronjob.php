    ...
    public function pembiayaan()
    {
        ...
        $hasil1 = $this->db->query(
            "SELECT t1.*,t2.nama,t3.nama_pegawai FROM tb_pembiayaan AS t1 
    								...
    								INNER JOIN pegawai AS t3 ON t1.nomor_ao=t3.nip WHERE t1.status=0"
        )->result();
        foreach ($hasil1 as $row1) {
            ...
            $hasil = $this->db->query(
                "SELECT t1.*,sum(accounttrans_value) AS jlh FROM tb_accounttrans AS t1
					...
					`accounttrans_type`='01' AND `accounttrans_date` <='$tgl2' GROUP BY accounttrans_listid ORDER BY `accounttrans_value` ASC"
            )->result();
            $pokok = 0;
            $margin = 0;
            $i=0;

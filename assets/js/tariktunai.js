 ...
 */
    ...
    $('#savetarik').click(function() {
        ...
        if (hasil['isi'] != "invalid") {
            ...
            norek = $("#form_tariktunai input[name='nomor_rekening']").val();
            jurnal = ajak('base/tabungan/jurnal', '&norek='+norek);
            saldo = ajak('base/tabungan/saldo','&id='+ norek+'&jurnal='+jurnal);
            ...
            if(limit == "no"){
                ...
                if (respon[0] == "1") {
                	...
                	$('#nomoraccount',ctkframe.document).html(respon[3]);
                    window.ctkframe.print();
                    setTimeout(function(){window.location.href = "/bmt/tariktunai"}, 1000);
                } else {
                    showinfo("Error : " + respon[0]);
                }
            }
    ...
    function(hal,juml,json) {
        ...
        for(i = 0; i < json['alldata'].length; i++) {
            ...
            jnstab = ajak('base/tabungan/single_jnstab','&jnstab='+json['alldata'][i].jenis_simpanan);
            //
